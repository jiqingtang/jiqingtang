...
function! s:closeTree()
    ...
    if winnr("$") != 1
        let pre_winx = getwinposx()
        let pre_winy = getwinposy()
        call s:exec(s:getTreeWinNum() . " wincmd w")
        close
        call s:exec("wincmd p")
        let &columns = &columns - (g:NERDTreeWinSize + 1)
        if pre_winx != getwinposx() || pre_winy != getwinposy()
            call s:exec("winpos " . pre_winx . " " . pre_winy)
        endif
    else
        close
    endif
...
function! s:createTreeWin()
    ...
    let &columns = &columns + splitSize + 1
